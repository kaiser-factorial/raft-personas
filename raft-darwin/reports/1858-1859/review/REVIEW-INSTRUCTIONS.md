# Full-source review: 1858–1859

The research endpoint is 24 November 1859, but this assignment covers only its
specified 1858–1859 packet. Work backwards from Charles Robert Darwin's outgoing
...
are dating investigations, not admitted 1858–1859 persona material. Seek an
...
```
/Users/corinakaiser/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 scripts/period_review.py --period 1858-1859 --batch batch-01 --offset 0 --limit 2
```
...
```
/Users/corinakaiser/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 scripts/period_review.py --period 1858-1859 --validate reports/1858-1859/review/reviews/batch-NN.json
```
...
The completed 1851–1855 primary checkpoint is reports/research/1851-1855-checkpoint.primary.json. The additive 1843 Lindley reverse reply is in data/annotations/reconciliation/1843-lindley-reverse.primary.json; earlier prose claimed it existed in a graph where it was absent. Do not recount it.

Each summary must describe substantive full-source content. Neither copied catalogue synopsis nor sampled beginning/end paragraphs establish a full reading. A standalone heading with following content is not a missing passage. Institution minutes, translations, reported quotations, drafts and literal historical text quoted in apparatus retain their exact voices and extent.

The completed1856–1857 checkpoint is reports/research/1856-1857-checkpoint.primary.json. All raw-date-boundary records remain evidence only until independently placed. The final endpoint is inclusive1859-11-24; later reception sources cannot become prepublication knowledge.1079,2065,1819,2179 are supplemental third-party context, never Darwin voice. See supplemental-context.primary.json and its pinned admissions.

Preserve authoritative additive corrections in reports/research/work_status.json. Do not generate summaries or claim full reading from sampled openings/endings. Read substantive complete sources and report specific content and uncertainty; no canned nearest-date matches or blanket negative assertions. A demonstrable partial response is eligible as a scoped relationship even if another prompt is missing.
